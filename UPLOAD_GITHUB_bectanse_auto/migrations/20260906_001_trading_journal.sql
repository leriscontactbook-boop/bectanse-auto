...
ALTER TABLE trading_subscriptions ADD COLUMN IF NOT EXISTS product TEXT NOT NULL DEFAULT 'JOURNAL';
ALTER TABLE trading_subscriptions ADD COLUMN IF NOT EXISTS stripe_price_id TEXT NOT NULL DEFAULT '';
ALTER TABLE trading_subscriptions ADD COLUMN IF NOT EXISTS cancel_at_period_end BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE trading_subscriptions ADD COLUMN IF NOT EXISTS payment_failed_at TIMESTAMPTZ;
ALTER TABLE trading_subscriptions ALTER COLUMN plan SET DEFAULT 'JOURNAL_PRO';
UPDATE trading_subscriptions SET plan='JOURNAL_PRO',subscription_status='inactive',updated_at=NOW() WHERE plan='FREE';
CREATE UNIQUE INDEX IF NOT EXISTS trading_subscriptions_stripe_subscription_idx ON trading_subscriptions (stripe_subscription_id) WHERE stripe_subscription_id <> '';
CREATE INDEX IF NOT EXISTS trading_subscriptions_customer_idx ON trading_subscriptions (stripe_customer_id) WHERE stripe_customer_id <> '';
...
CREATE INDEX IF NOT EXISTS product_entitlements_active_idx ON product_entitlements (user_id, feature_key, valid_until) WHERE status = 'ACTIVE';
...
CREATE INDEX IF NOT EXISTS stripe_journal_events_status_idx ON stripe_journal_events (status, created_at DESC);
...
INSERT INTO trading_credentials (trading_account_id,encrypted_password,nonce,auth_tag,key_version)
SELECT id,encrypted_password,credential_nonce,credential_tag,1 FROM trading_accounts
WHERE encrypted_password IS NOT NULL AND credential_nonce IS NOT NULL AND credential_tag IS NOT NULL
...
CREATE INDEX IF NOT EXISTS trading_accounts_user_idx ON trading_accounts (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS trading_accounts_due_idx ON trading_accounts (status, last_successful_sync_at) WHERE status <> 'DISCONNECTED';
...
CREATE INDEX IF NOT EXISTS trading_deals_account_time_idx ON trading_deals (trading_account_id, executed_at DESC);
CREATE INDEX IF NOT EXISTS trading_deals_account_position_idx ON trading_deals (trading_account_id, mt5_position_id, executed_at) WHERE is_trading_deal = TRUE;
CREATE INDEX IF NOT EXISTS trading_deals_calendar_idx ON trading_deals (trading_account_id, executed_at) WHERE is_trading_deal = TRUE;
...
CREATE INDEX IF NOT EXISTS trading_snapshots_account_time_idx ON trading_account_snapshots (trading_account_id, captured_at DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS trading_sync_one_active_account_idx ON trading_sync_jobs (trading_account_id) WHERE status IN ('PENDING','LEASED','RUNNING','RETRY');
DROP INDEX IF EXISTS trading_sync_claim_idx;
CREATE INDEX IF NOT EXISTS trading_sync_claim_idx ON trading_sync_jobs (status, not_before, priority DESC, created_at) WHERE status IN ('PENDING','RETRY');
...
CREATE INDEX IF NOT EXISTS trading_sync_runs_account_idx ON trading_sync_runs (trading_account_id, started_at DESC);
...
CREATE INDEX IF NOT EXISTS trading_reconciliation_account_idx ON trading_reconciliation_reports (trading_account_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS trading_sync_events_account_idx ON trading_sync_events (trading_account_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS trading_audit_user_idx ON trading_audit_logs (user_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS trading_workers_seen_idx ON trading_workers (last_seen_at DESC);
...
CREATE INDEX IF NOT EXISTS trading_worker_nonces_created_idx ON trading_worker_nonces (created_at);
